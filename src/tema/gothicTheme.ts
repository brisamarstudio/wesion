/**
 * ⚠️ PALETTE SEMANTICA ALZATA DI SATURAZIONE (31/08/2026), E NON PER GUSTO.
 *
 * Il gothic originale dipinge stati e badge con pastelli tenui — sage moss,
 * dusty rose, aged gold. Su una pagina di prosa sono belli; sull'elenco
 * aziende, che sono venticinque righe fitte con un punteggio e un'etichetta
 * per riga, diventavano tutti lo stesso beige: «sembrano gli zombie», detto da
 * chi lo usa. Il badge di un lead a 95 e quello di «senza sito» devono
 * separarsi da un metro, perche' quella colonna serve a scegliere chi chiamare
 * per primo — e' l'unico lavoro di quella schermata.
 *
 * Cambiati SOLO i quattro fondi categoriali e i tre colori di stato. Il resto
 * del tema — sfondi, testi, tipografia — e' quello di partenza: il colore
 * squilla dove porta informazione e tace dappertutto altrove.
 *
 * ⚠️ QUESTO E' IL SORGENTE. L'app legge `gothic.js` + `gothic.css`: dopo aver
 * toccato questo file serve `npm run astryx -- theme build src/tema/gothicTheme.ts`,
 * o non cambia niente e ci si passa mezz'ora.
 */
...
export const gothicTheme = defineTheme({
  ...
  tokens: {
    ...
    '--color-success': '#7ed957', // verde vivo (era sage moss #b3c79a)
    '--color-success-muted': '#7ed957',
    '--color-error': '#ff7a6e', // corallo vivo (era dusty rose #c6a6a2)
    '--color-error-muted': '#ff7a6e',
    '--color-warning': '#f5c542', // oro vivo (era aged gold #d3c490)
    '--color-warning-muted': '#f5c542',
    ...
    // Yellow (aged gold)
    '--color-background-yellow': '#f5c542',
  ...
  components: {
    ...
    /**
     * Le linguette a SCATOLETTA, non sottolineate.
     *
     * ⚠️ Astryx le disegna con un filetto sotto quella scelta: elegante, e
     * invisibile. Il 31/08/2026 chi usava l'elenco aziende non le ha proprio
     * riconosciute come un comando — «ma ste tab dove sono?» — e stava
     * guardando la riga in cui erano. Prima le ho fatte piu' grandi: non e'
     * bastato, perche' il problema non era la dimensione ma la FORMA. Una
     * linguetta si riconosce se ha un bordo attorno: e' la forma che tutti
     * hanno imparato dai browser.
     *
     * Quella scelta ha il fondo della pagina e il bordo che continua nel
     * contenuto; le altre stanno un gradino indietro, sul fondo smorzato.
     */
    tab: {
      base: {
    ...
    banner: {
      ...
      /**
       * ⚠️ NE' `--color-text-red` NE' `--color-background-red` QUI (01/09/2026,
       * corretto due volte). Primo tentativo: testo scuro su
       * `--color-background-red` (il corallo dei badge) — matematicamente
       * sopra soglia (~7,4:1) ma percepito IDENTICO a prima, perche' resta
       * scuro-su-chiaro come l'originale marrone-su-corallo (~5,2:1): due
       * toni scuri diversi non si distinguono a colpo d'occhio.
       * `#7a1f1f` e' un rosso davvero scuro, in chiaro (non un token: la lista
       * dei token del tema e' chiusa, e questo serve SOLO qui) apposta solo
       * per questo banner — i badge restano corallo, li' il testo scuro basta
       * e cambiarlo avrebbe spostato un problema che non c'era.
       * Contrasto con `--color-on-dark`: ~8,9:1, e si VEDE la differenza.
       */
      'status:error': {
        backgroundColor: '#7a1f1f',
        '--color-text-primary': 'var(--color-on-dark)',
        '--color-text-secondary': 'var(--color-on-dark)',
        '--color-error': 'var(--color-on-dark)',
      },
    },
